"use client";
// src/sections/Skills.tsx — Image 2 style: Oval skill icons + animated bars + pill grid
...
// ── Oval skill role cards (Image 2 top section style) ──
const ROLE_SKILLS = [
  { icon: "⚡", label: "Frontend\nDeveloper",  color: "#00f5ff", glow: "rgba(0,245,255,0.25)"   },
  { icon: "🤖", label: "AI\nEngineer",         color: "#a855f7", glow: "rgba(168,85,247,0.25)"  },
  { icon: "🛠",  label: "Backend\nDeveloper",   color: "#34d399", glow: "rgba(52,211,153,0.25)"  },
  { icon: "🗄",  label: "Database\nEngineer",   color: "#f472b6", glow: "rgba(244,114,182,0.25)" },
  { icon: "🚀",  label: "DevOps\nEngineer",     color: "#fbbf24", glow: "rgba(251,191,36,0.25)"  },
];
...
const CARD_HOVER: Record<string, string> = {
  cyan:   "hover:border-[#00f5ff]/28 hover:bg-[#00f5ff]/[0.03]",
  purple: "hover:border-[#a855f7]/28 hover:bg-[#a855f7]/[0.03]",
  green:  "hover:border-[#34d399]/28 hover:bg-[#34d399]/[0.03]",
  pink:   "hover:border-[#f472b6]/28 hover:bg-[#f472b6]/[0.03]",
};
...
export default function Skills() {
  ...
  return (
    <SectionWrapper id="skills" alt>
      ...
      {/* ── Oval Role Cards (Image 2 style) ── */}
      <div ref={roleRef} className="flex justify-center gap-5 mt-12 flex-wrap">
        {ROLE_SKILLS.map((role, i) => (
          ...
          >
            {/* Oval card */}
            <div
              className="w-[110px] h-[140px] rounded-[55px] flex items-center justify-center
                border transition-all duration-300 relative overflow-hidden"
              style={{
                background: `radial-gradient(circle at 50% 30%, ${role.glow}, rgba(255,255,255,0.03) 70%)`,
            ...
            >
              ...
              <span className="text-[40px] relative z-10">{role.icon}</span>
            </div>
            {/* Label */}
            <p className="text-[12.5px] font-bold text-center text-white/70 leading-tight whitespace-pre-line">
              {role.label}
            </p>
          </motion.div>
        ))}
      ...
      <div ref={catRef} className="grid grid-cols-2 gap-4">
        {skillCategories.map((cat, i) => (
          <motion.div
            key={cat.title}
            initial={{ opacity: 0, y: 22 }}
            animate={catIn ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.6, delay: i * 0.1, ease: [0.22, 0.68, 0, 1.2] }}
            className={`glass-card card-shimmer rounded-[20px] p-5
              transition-all duration-300 ${CARD_HOVER[cat.theme]}`}
          >
            <div className="w-full h-[2px] rounded-full mb-4 opacity-40"
              style={{ background: `linear-gradient(90deg,${PILL_THEME[cat.theme].match(/#[a-f0-9]+/i)?.[0] ?? "#00f5ff"},transparent)` }} />
            <h3 className="font-display text-[14px] font-bold mb-4 flex items-center gap-2">
      ...
      <div ref={barRef} className="mt-14">
        <p className="inline-flex items-center gap-2 text-[10.5px] font-black
          tracking-[0.14em] uppercase text-[#00f5ff] mb-7
            ...
            <div key={ci}>
              {col.map((bar, bi) => (
                <motion.div key={bar.name}
                  ...
                  animate={barIn ? { opacity: 1, x: 0 } : {}}
                  transition={{ duration: 0.5, delay: bi * 0.09 }}
                  className="mb-5 group"
                >
                  <div className="flex justify-between mb-1.5">
                    <span className="text-[13px] font-semibold group-hover:text-white transition-colors">
                      {bar.name}
                    </span>
                    <span className="text-[12px] font-black text-[#00f5ff] font-mono">
